...
### Added
- **轻 toast 反馈**：已发送 / 已复制 / 已存入知识库等瞬时结果改为底部浮层提示（1.8s 自动消失），
  不再借用状态栏或弹窗
- **内联错误条**：读取失败 / AI 超时 / 改写失败在草稿框上方内联呈现 + 重试按钮，
  移除打断式弹窗（破坏性确认保留）；辅助功能权限缺失时内联引导「去开启」前置到主路径
- **改写可撤销**：`DraftHistory` 栈记录改写/重新生成前的草稿，「撤销」按钮 + ⌘Z 逐级回退
  （业内 Grammarly/Notion AI 范式，AI 改写从破坏性替换变为可逆迭代）
- **AI 来源 caption**：生成完成后草稿框下方标注「ⓘ 据 N 条会话 (+ 知识库) 生成 · 发送前请确认」
  （对齐 Intercom/Zendesk 坐席副驾的来源可追溯实践）
- **跨分组搜索**：话术搜索非空时检索全部分组，结果卡片标注来源分组（Raycast 搜索优先范式）
- 测试：`tests/test_gui_panel_ui.py`（DraftHistory / make_source_caption / filter_phrases）

### Changed
- **草稿台即主界面（spec v2 P0）**：移除「话术 / AI 助手」双模式切换行，
  「话术」入口上移顶栏（与 ⋯ 菜单并列），点击在草稿台 ⇄ 话术间切换
- **状态点并入对象选择器**：顶栏左侧 = ● 状态点 + IM 选择器，删除常驻状态文字行；
  点状态点以 toast 查看状态详情
- **聊天上下文折叠**：常驻 100px 文本框改为单行摘要（▸ 已读取 N 条 · 时间），
  点击展开/收起，释放高度给草稿框
- **视觉 Token 落地（spec v2 P2）**：主色 Indigo `#4F46E5` → 收敛蓝 `#2B5CE6`；
  状态色对齐 spec（检测中=中性灰）；全部 emoji 替换为线性符号/纯文字；
  弹窗标题栏统一近白 + 1px 分隔线；字号收敛 13/12/11、按钮圆角 8 / 输入框 10；
  知识库状态行去彩色底改中性描边
